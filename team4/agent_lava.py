...
class agent():
    ...
    def load_weights(self):
        self.model.load_state_dict(torch.load('model/DQN_0.005_0.95_50.0_1.0_10000.pt'))
    ...
    def action(self, state, epi): # state: int 또는 numpy.array
        ...
        if self.noise1 != None:
            ...
            # qval_ += np.random.rand(self.nA) * self.noise1/(epi+1)

        a = np.argmax(qval_)

        return a

    def update(self, new_state, action, reward, epi):
        ...
        if self.noise2 != None:
            ...
            # newQ += torch.randn(self.nA) * self.noise2/(epi+1) 

        maxQ = torch.max(newQ) 
        if reward == -0.01: # 이 값 바뀌게 되면 어떡함? 
